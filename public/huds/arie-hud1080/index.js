...
function fillObserved(player) {
    ...
    $("#observed_name span").text(player.real_name);
    ...
    $("#obs_armor .bar .fill").attr("style", "width:" + statistics.armor + "%");
...
function fillPlayer(player,nr, side, max){
    ...
    if(statistics.health == 0) {
        $top.find("#hp_p").html("<img src='/files/img/death.png' class='death_big'>");
    } else {
...
//    $bottom.find(".deaths").text(statistics.deaths);
    ...
    if(statistics.round_kills > 0){
        ...
        $top.find("#weapon_icon").prepend($("<img />").attr("src", "/files/img/death.png").addClass("death").css("float", side)).prepend($("<div></div>").text(statistics.round_kills).css(img_css));
    }
